...
const skills = [
  { id: 'prompt', icon: '🔮', title: '提示词工程与大模型应用', tag: '核心能力', tagBg: 'rgba(62,154,184,0.12)', tagColor: '#3e9ab8', tagBorder: 'rgba(62,154,184,0.25)', color: '#3e9ab8', matchProject: 3,
    items: ['精通多场景Prompt设计与迭代调优，沉淀20+可复用模板', '结构化输出准确率95%+，覆盖电商、运维、安全、数据四大场景', '熟练掌握大模型API调用、Function Calling、多轮对话逻辑'],
    keywords: ['Prompt Design', 'Function Calling', '结构化输出', '模板库20+'] },
  { id: 'automation', icon: '⚡', title: 'AI自动化开发', tag: '工具链', tagBg: 'rgba(72,168,152,0.12)', tagColor: '#48a898', tagBorder: 'rgba(72,168,152,0.25)', color: '#48a898', matchProject: 1,
    items: ['独立搭建AI Agent智能体，结合Python脚本实现自动化落地', '熟练使用Claude Code，完成需求拆解→开发→测试→部署全闭环', '代表成果：报表效率提升96%，巡检效率提升60%', 'GitHub持续积累：AI工具、自动化脚本、安全巡检等项目开源实践'],
    keywords: ['AI Agent', 'Python', 'Claude Code', 'GitHub', '全闭环开发'] },
  { id: 'security', icon: '🛡️', title: '网络安全与运维', tag: '安全底盘', tagBg: 'rgba(94,94,168,0.12)', tagColor: '#5e5ea8', tagBorder: 'rgba(94,94,168,0.25)', color: '#5e5ea8', matchProject: 0,
    items: ['百台级服务器运维，生产环境可用性99.9%+，零操作事故', '精通Web漏洞攻防、告警分析、应急响应、钓鱼邮件防护', '熟悉防火墙/WAF/IDS/堡垒机，参与等保合规与AI安全评估'],
    keywords: ['Web攻防', '应急响应', '等保合规', 'AI安全评估'] },
  { id: 'ecommerce', icon: '🛒', title: '电商技术赋能', tag: '探索中', tagBg: 'rgba(176,112,80,0.12)', tagColor: '#b07050', tagBorder: 'rgba(176,112,80,0.25)', color: '#b07050', matchProject: 4,
    items: ['独立运营闲鱼店铺，掌握选品、上架、转化全流程逻辑', '用AI与自动化能力优化电商运营全链路', '关注跨境电商，研究AI在外贸选品、客服、营销中的落地'],
...
export default function Skills({ onSkillClick }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Big English title — slides from right, heavy
      gsap.fromTo(enTitleRef.current,
        { x: 100, scaleX: 1.3, opacity: 0, filter: 'blur(6px)' },
        {
  ...
  return (
    <section ref={sectionRef} id="skills" className="section">
      <div className="container">
        {/* Big English title */}
        <div ref={enTitleRef} style={{ marginBottom: 48, overflow: 'hidden', textAlign: 'center' }}>
          <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <p className="section-subtitle" style={{ margin: '0 auto' }}>
              安全底座 × AI能力 × 业务场景<br />
              <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>下滑堆叠 · 点击探索项目</span>
            </p>
          </div>
        </div>

        <ScrollStack itemDistance={60} itemScale={0.02} itemStackDistance={20} stackPosition="20%" baseScale={0.9} useWindowScroll={true}>
          {skills.map((s, i) => (
            <ScrollStackItem key={i}>
              <div data-cursor="glow" onClick={() => handleClick(s)} style={{ padding: '36px 32px', background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius)', cursor: 'pointer', height: '100%', transition: 'border-color 0.3s' }}
              ...
              >
                ...
                {s.id === 'automation' && (
                  <a href="https://github.com/sun6792" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                  ...
                  >⌘ github.com/sun6792 <span style={{ fontSize: '0.8rem' }}>↗</span></a>
                )}
